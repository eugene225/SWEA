#include <stdio.h>
#include <string.h>

typedef enum
{
	CMD_INIT,
	CMD_ADD,
	CMD_DELETE,
	CMD_CHANGE,
	CMD_SEARCH
} COMMAND;

typedef enum
{
	NAME,
	NUMBER,
	BIRTHDAY,
	EMAIL,
	MEMO
} FIELD;

typedef struct
{
	int count;
	char str[20];
} RESULT;

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

extern void InitDB();
extern void Add(char* name, char* number, char* birthday, char* email, char* memo);
extern int Delete(FIELD field, char* str);
extern int Change(FIELD field, char* str, FIELD changefield, char* changestr);
extern RESULT Search(FIELD field, char* str, FIELD returnfield);

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

static int dummy[100];
static int Score, ScoreIdx;
static char name[20], number[20], birthday[20], email[20], memo[20];

static char lastname[10][5] = { "kim", "lee", "park", "choi", "jung", "kang", "cho", "oh", "jang", "lim" };
static int lastname_length[10] = { 3, 3, 4, 4, 4, 4, 3, 2, 4, 3 };

static int mSeed;
static int mrand(int num)
{
	mSeed = mSeed * 1103515245 + 37209;
	if (mSeed < 0) mSeed *= -1;
	return ((mSeed >> 8) % num);
}

static void make_field(int seed)
{
	int name_length, email_length, memo_length;
	int idx, num;

	mSeed = (unsigned int)seed;

	name_length = 6 + mrand(10);
	email_length = 10 + mrand(10);
	memo_length = 5 + mrand(10);

	num = mrand(10);
	for (idx = 0; idx < lastname_length[num]; idx++) name[idx] = lastname[num][idx];
	for (; idx < name_length; idx++) name[idx] = 'a' + mrand(26);
	name[idx] = 0;

	for (idx = 0; idx < memo_length; idx++) memo[idx] = 'a' + mrand(26);
	memo[idx] = 0;

	for (idx = 0; idx < email_length - 6; idx++) email[idx] = 'a' + mrand(26);
	email[idx++] = '@';
	email[idx++] = 'a' + mrand(26);
	email[idx++] = '.';
	email[idx++] = 'c';
	email[idx++] = 'o';
	email[idx++] = 'm';
	email[idx] = 0;

	idx = 0;
	number[idx++] = '0';
	number[idx++] = '1';
	number[idx++] = '0';
	for (; idx < 11; idx++) number[idx] = '0' + mrand(10);
	number[idx] = 0;

	idx = 0;
	birthday[idx++] = '1';
	birthday[idx++] = '9';
	num = mrand(100);
	birthday[idx++] = '0' + num / 10;
	birthday[idx++] = '0' + num % 10;
	num = 1 + mrand(12);
	birthday[idx++] = '0' + num / 10;
	birthday[idx++] = '0' + num % 10;
	num = 1 + mrand(30);
	birthday[idx++] = '0' + num / 10;
	birthday[idx++] = '0' + num % 10;
	birthday[idx] = 0;
}

static void cmd_init()
{
	scanf("%d", &ScoreIdx);

	InitDB();
}

static void cmd_add()
{
	int seed;
	scanf("%d", &seed);

	make_field(seed);

	Add(name, number, birthday, email, memo);
}

static void cmd_delete()
{
	int field, check, result;
	char str[20];
	scanf("%d %s %d", &field, str, &check);

	result = Delete((FIELD)field, str);
	if (result != check)
		Score -= ScoreIdx;
}

static void cmd_change()
{
	int field, changefield, check, result;
	char str[20], changestr[20];
	scanf("%d %s %d %s %d", &field, str, &changefield, changestr, &check);

	result = Change((FIELD)field, str, (FIELD)changefield, changestr);
	if (result != check)
		Score -= ScoreIdx;
}

static void cmd_search()
{
	int field, returnfield, check;
	char str[20], checkstr[20];
	scanf("%d %s %d %s %d", &field, str, &returnfield, checkstr, &check);

	RESULT result = Search((FIELD)field, str, (FIELD)returnfield);

	if (result.count != check || (result.count == 1 && (strcmp(checkstr, result.str) != 0)))
		Score -= ScoreIdx;
}

static void run()
{
	int N;
	scanf("%d", &N);
	for (int i = 0; i < N; i++)
	{
		int cmd;
		scanf("%d", &cmd);
		switch (cmd)
		{
		case CMD_INIT:   cmd_init();   break;
		case CMD_ADD:    cmd_add();    break;
		case CMD_DELETE: cmd_delete(); break;
		case CMD_CHANGE: cmd_change(); break;
		case CMD_SEARCH: cmd_search(); break;
		default: break;
		}
	}
}

static void init()
{
	Score = 1000;
	ScoreIdx = 1;
}

int main()
{
	setbuf(stdout, NULL);
	//freopen("sample_input.txt", "r", stdin);

	int T;
	scanf("%d", &T);

	int TotalScore = 0;
	for (int tc = 1; tc <= T; tc++)
	{
		init();

		run();

		if (Score < 0)
			Score = 0;

		TotalScore += Score;
		printf("#%d %d\n", tc, Score);
	}
	printf("TotalScore = %d\n", TotalScore);

	return 0;
}

#define _CRT_SECURE_NO_WARNINGS
//#include < bits/stdc++.h >
#include < unordered_map >

using namespace std;

typedef enum {
	NAME,
	NUMBER,
	BIRTHDAY,
	EMAIL,
	MEMO
} FIELD;

typedef struct {
	int count;
	char str[20];
} RESULT;

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
struct Node {
	char name[20];
	char number[20];
	char birthday[20];
	char email[20];
	char memo[20];
};

Node record[50010];
int nodeCnt;

unordered_map< string, vector< Node*>> DB[5];

Node* getNode(char* name, char* number, char* birthday, char* email, char* memo) {
	strcpy(record[nodeCnt].name, name);
	strcpy(record[nodeCnt].number, number);
	strcpy(record[nodeCnt].birthday, birthday);
	strcpy(record[nodeCnt].email, email);
	strcpy(record[nodeCnt].memo, memo);
	return &record[nodeCnt++];
}

// 0
void InitDB() {
	for (int i = 0; i < 5; i++) {
		DB[i].clear();
	}
	nodeCnt = 0;
}

// 1
void Add(char* name, char* number, char* birthday, char* email, char* memo) {
	Node* temp = getNode(name, number, birthday, email, memo);
	DB[0][name].push_back(temp);
	DB[1][number].push_back(temp);
	DB[2][birthday].push_back(temp);
	DB[3][email].push_back(temp);
	DB[4][memo].push_back(temp);
}

// 2 str이 여러개 있을수도 있음
int Delete(FIELD field, char* str) {
	int ans = DB[field][str].size();
	if (ans == 0)
		return ans;

	for (int i = 0; i < DB[field][str].size(); i++) {
		Node* m = DB[field][str][i];
		char temp[5][20];
		strcpy(temp[0], m->name);
		strcpy(temp[1], m->number);
		strcpy(temp[2], m->birthday);
		strcpy(temp[3], m->email);
		strcpy(temp[4], m->memo);

		for (int i = 0; i < 5; i++) {
			if (field == i)
				continue;

			if (find(DB[i][temp[i]].begin(), DB[i][temp[i]].end(), m) != DB[i][temp[i]].end()) {
				auto n = find(DB[i][temp[i]].begin(), DB[i][temp[i]].end(), m);
				DB[i][temp[i]].erase(n);
			}
		}
	}

	DB[field][str].clear();

	return ans;
}

// 3    str이 여러개있을수도 있음
int Change(FIELD field, char* str, FIELD changefield, char* changestr) {
	int ans = DB[field][str].size();
	if (ans == 0)
		return ans;

	for (auto m : DB[field][str]) {
		char original[20];
		switch (changefield) {
		case 0:
			strcpy(original, m->name);
			strcpy(m->name, changestr);
			break;
		case 1:
			strcpy(original, m->number);
			strcpy(m->number, changestr);
			break;
		case 2:
			strcpy(original, m->birthday);
			strcpy(m->birthday, changestr);
			break;
		case 3:
			strcpy(original, m->email);
			strcpy(m->email, changestr);
			break;
		case 4:
			strcpy(original, m->memo);
			strcpy(m->memo, changestr);
			break;
		}

		auto n = find(DB[changefield][original].begin(), DB[changefield][original].end(), m);
		DB[changefield][original].erase(n);
		DB[changefield][changestr].push_back(m);
	}

	return ans;
}

// 4    str이 여러개일수도 있음
RESULT Search(FIELD field, char* str, FIELD ret_field) {
	RESULT result;
	result.count = DB[field][str].size();

	if (result.count == 1) {
		for (auto m : DB[field][str]) {
			switch (ret_field) {
			case 0:
				strcpy(result.str, m->name);
				break;
			case 1:
				strcpy(result.str, m->number);
				break;
			case 2:
				strcpy(result.str, m->birthday);
				break;
			case 3:
				strcpy(result.str, m->email);
				break;
			case 4:
				strcpy(result.str, m->memo);
				break;
			}
		}
	}
	else
		strcpy(result.str, "");

	return result;
}